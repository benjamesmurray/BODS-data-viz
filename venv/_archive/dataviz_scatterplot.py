...
def calculate_figsize(width_ratio=1, height_ratio=0.8):
    monitor = get_monitors()[0]
    width = monitor.width * width_ratio
    height = monitor.height * height_ratio
    return width / 80, height / 80  # Divide by 80 to convert pixels to inches
...
def create_fig1(grouped, current_directory):
    # Filter the data
    ...
    fig1, ax = plt.subplots(figsize=calculate_figsize())
    ...
    img_data = BytesIO()
    plt.tight_layout()
    fig1.savefig(img_data, format='png', dpi=900, bbox_inches='tight')
    img_data.seek(0)
    plt.close(fig1)  # Close the figure to free up memory
    return fig1


def create_fig2(grouped, current_directory):
    grouped_0 = grouped[grouped["Attention Percentage"] == 0]


    # Create scatterplots for the 0% and 100% attention percentage cases
    fig2, ax2 = plt.subplots(figsize=calculate_figsize())
    ...
    ax2.set_xlabel("Total Number of Services")
    ax2.set_ylabel("Percentage of Services Needing Attention")
    ax2.set_xscale('log')
    ax2.xaxis.set_major_formatter(plt.ScalarFormatter())
    img_data = BytesIO()
    plt.tight_layout()
    fig2.savefig(img_data, format='png', dpi=900, bbox_inches='tight')
    img_data.seek(0)
    plt.close(fig2)  # Close the figure to free up memory
    return fig2


def create_fig3(grouped, current_directory):
    grouped_100 = grouped[grouped["Attention Percentage"] == 100]

    fig3, ax3 = plt.subplots(figsize=calculate_figsize())
    scatter_100 = sns.scatterplot(
        data=grouped_100,
        x="Total Services",
        y="Attention Percentage",
        hue="Attention Percentage",
    ...
    ax3.set_xlabel("Total Number of Services")
    ax3.set_ylabel("Percentage of Services Needing Attention")
    ax3.set_xscale('log')
    ax3.xaxis.set_major_formatter(plt.ScalarFormatter())
    img_data = BytesIO()
    plt.tight_layout()
    fig3.savefig(img_data, format='png', dpi=900, bbox_inches='tight')
    img_data.seek(0)
